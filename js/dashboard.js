...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const loadTickets = async () => {
        try {
            const resp = await fetch('api/dashboard/get_tickets.php', { credentials: 'same-origin' });
            const data = await resp.json();
            const list = document.querySelector('.ticket-list');
            if (list && data.status === 'success') {
                if (data.tickets.length === 0) {
                    list.innerHTML = '<p class="text-center p-4">No support tickets found.</p>';
                    return;
                }
                list.innerHTML = ''; // Clear existing content
                data.tickets.forEach(t => {
                    const div = document.createElement('div');
                    div.className = `ticket-item ${t.is_read ? '' : 'unread'}`;
                    div.innerHTML = `
                        <div class="ticket-status badge badge-${t.status === 'open' ? 'warning' : (t.status === 'resolved' ? 'success' : 'info')}">${t.status}</div>
                        <div class="ticket-details">
                            <h4>${t.subject}</h4>
                            <p class="ticket-meta">Ticket #${t.ticket_number} • Opened ${new Date(t.created_at).toLocaleDateString()} • ${t.category}</p>
                        </div>
                        <a href="#" class="btn-text view-ticket-btn" data-id="${t.id}">View</a>
                    `;
                ...
                document.querySelectorAll('.view-ticket-btn').forEach(btn => {
                    btn.onclick = (e) => {
                        e.preventDefault();
                        openTicketDetails(btn.getAttribute('data-id'));
                    };
                });
            }
    ...
    const initProfileTabs = () => {
        const profileLinks = document.querySelectorAll('.profile-side-link');
        const profilePanes = document.querySelectorAll('.profile-pane');
        console.log('Profile Tabs Found:', profileLinks.length);

        profileLinks.forEach(link => {
            link.addEventListener('click', () => {
                const target = link.getAttribute('data-profile-tab');
                console.log('Switching to profile tab:', target);
    ...
    const loadProfile = async () => {
        try {
            ...
            if (data.status === 'success') {
                const u = data.user;
                console.log('User data loaded:', u.full_name);
                ...
                console.log('Elements found - displayName:', !!displayName, 'nameInput:', !!nameInput);

                if (headerName) headerName.textContent = u.full_name;
                if (displayName) {
                    displayName.textContent = u.full_name;
                    console.log('Updated displayName to:', u.full_name);
                }
                ...
                console.log('Avatars found - mainAvatarImg:', !!mainAvatarImg, 'headerAvatarImg:', !!headerAvatarImg);
    ...
    const initFormHandlers = () => {
        ...
        if (profileForm) {
            console.log('Profile form found, binding submit...');
...
// Global logout handler removed - now in main.js
