...
if (!$device_secret) {
    ...
    echo json_encode(['message' => 'Missing device_secret', 'success' => false]);
    exit;
}
...
try {
    ...
    if (!empty($device['user_id']) && !empty($device['user_device_name'])) {
        if ((int)$device['user_id'] === (int)$user_id) {
            ...
            $stmt->execute([NULL, NULL, $device['id']]);

            // $isSuccess = publishToEMQX("device/$device_secret/auth", $token, 1);
            echo json_encode(['message' => 'Device Removed successfully', 'success' => true]);
        } else {
    ...
    echo json_encode(['message' => 'Device not registered by you', 'success' => false]);
} catch (PDOException $e) {
    http_response_code(500);
    echo json_encode(['message' => 'Failed to remove device', 'error' => $e->getMessage(), 'success' => false]);
}
?>
